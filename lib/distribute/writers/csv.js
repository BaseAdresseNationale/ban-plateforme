...
async function createWriter(outputPath, departement) {
  ...
  let lieuxDitsStreamAcceptsWrite = true

  return {
    async writeAdresses(adresses) {
      adresses.forEach(adresse => {
        if (adresse.type === 'lieu-dit') {
          lieuxDitsStreamAcceptsWrite = lieuxDitsStream.write(lieuDitToRow(adresse))
          return
        }

        if (adresse.numero && adresse.position && adresse.lon && adresse.lat) {
          adressesStreamAcceptsWrite = adressesStream.write(adresseToRow(adresse))
        }
      })

      await Promise.all([
        adressesStreamAcceptsWrite || new Promise(resolve => adressesStream.once('drain', resolve)),
        lieuxDitsStreamAcceptsWrite || new Promise(resolve => lieuxDitsStream.once('drain', resolve))
      ])
    },
