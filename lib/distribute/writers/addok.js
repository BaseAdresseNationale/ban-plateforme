...
async function writeData({outputPath, departement, adresses, lieuxDits}) {
  const filePath = join(outputPath, `adresses-addok-${departement}.ndjson.gz`)
  ...
  const missingCommunes = communes.filter(c => c.departement === departement && !(c.code in adressesCommunes))
