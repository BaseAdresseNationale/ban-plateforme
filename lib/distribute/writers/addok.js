...
const {getCentroidFromPoints, derivePositionProps} = require('../../util/geo')
...
function getCenterProps(commune, numeros) {
  if (numeros.length > 0) {
    const centroid = getCentroidFromPoints(numeros.map(n => n.position))
    return derivePositionProps(centroid)
  }

  if (geo[commune.code]) {
    return derivePositionProps(point(geo[commune.code].center))
  }
...
function buildStreet(voie, forceAddOldCity = false) {
  ...
  const {lon, lat, x, y} = derivePositionProps(
    getCentroidFromPoints(voie.numeros.map(n => n.position))
  )
