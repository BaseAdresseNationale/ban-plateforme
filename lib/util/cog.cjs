...
const previousCommunesIndex = keyBy(previousCommunes.filter(c => ['arrondissement-municipal', 'commune-actuelle'].includes(c.type)), 'code')
...
function getCommune(codeCommune, withCommuneDelOrDie) {
  if (withCommuneDelOrDie) {
    ...
    if (commune) {
      const {codesPostaux, population} = previousCommunesIndex[codeCommune] || {}

      if (!commune.codesPostaux && codesPostaux) {
        commune.codesPostaux = codesPostaux
      }

      if (!commune.population && population) {
