...
function getCodesCommunes(codeDepartement) {
  return communes.filter(c => c.departement === codeDepartement && ['arrondissement-municipal', 'commune-actuelle'].includes(c.type)).map(c => c.code).filter(c => !['75056', '13055', '69123'].includes(c))
}
...
module.exports = {getMostRecentCommune, getCommuneActuelle, getCodesDepartements, getCodesMembres, getCodesCommunes, getCodeDepartement, getPLMCodeCommune, getNomCommune}
