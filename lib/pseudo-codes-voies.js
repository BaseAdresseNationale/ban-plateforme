...
async function loadPseudoCodesVoies(codeCommune) {
  ...
  const fileCodesVoies = await db.getCommune(codeCommune)

  return fileCodesVoies || []
}
...
async function createPseudoCodeVoieGenerator(codeCommune) {
  ...
  return {
    ...
    getDistance(idVoie, nomVoie) {
      ...
      const distance = leven(ctx.significantWordsString, item.stringContext.significantWordsString)
      console.log(`${ctx.significantWordsString} => ${item.stringContext.significantWordsString} | d:${distance}`)
      return distance
    },
