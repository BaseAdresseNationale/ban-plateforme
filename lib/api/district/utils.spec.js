...
describe('checkDistrictsRequest', () => {
  ...
  it('Rejects body with config on insert', async () => {
    const withConfig = districtMock.map(d => ({...d, config: {certificate: 'DISTRICT'}}))
    const districtsValidation = await checkDistrictsRequest(withConfig, 'insert')
    expect(districtsValidation?.isValid).toBe(false)
    expect(districtsValidation?.report?.message).toMatch(/district-config/)
  })
