...
// Export const getDistrictsFromCog = cog => District.findAll({where: {meta: {insee: {cog}}}, order: [[Sequelize.literal("(meta#>>'{insee, isMain}')::BOOLEAN"), 'DESC']], raw: true})
export const getDistrictsFromCog = cog => District.findAll({where: {
  [Op.or]: [
    {meta: {insee: {cog}}},
    {meta: {insee: {mainCog: cog}}},
  ]
}, order: [[Sequelize.literal('(meta#>>\'{insee, isMain}\')::BOOLEAN'), 'DESC']], raw: true})
