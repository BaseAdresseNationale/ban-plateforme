...
export const patchDistricts = async districts => {
  const bulkOperations = districts.map(async district => {
    ...
    // Update and merge config field
    if (districtRest.config) {
      const commonConfig = Object.entries(districtDB.config).filter(([key,]) => district.config[key] !== undefined).map(([key,]) => [key, district.config[key]])
      const notCommonConfigLeft = Object.entries(districtDB.config).filter(([key,]) => district.config[key] === undefined)
      const notCommonConfigRight = Object.entries(district.config).filter(([key,]) => districtDB.config[key] === undefined)
      districtRest.config = Object.fromEntries([...commonConfig, ...notCommonConfigLeft, ...notCommonConfigRight])
    }
