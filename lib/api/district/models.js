...
export const getDistrict = async districtID => District.findByPk(districtID, {raw: true})

export const getDistricts = async districtIDs => {
...
export const getDistrictsFromCog = cog => District.findAll({
  where: {
    [Op.or]: [
      {meta: {insee: {cog}}},
...
export const setDistricts = async districts => {
  for (const d of districts) {
    if (Object.prototype.hasOwnProperty.call(d, 'config')) {
      throw new Error(
        'District "config" cannot be sent via POST /district. Use PATCH /api/district-config/:districtId with Authorization token.'
      )
    }
  }
...
export const updateDistricts = async districts => {
  const promises = districts.map(async district => {
    if (Object.prototype.hasOwnProperty.call(district, 'config')) {
      throw new Error(
        'District "config" cannot be sent via PUT /district. Use PATCH /api/district-config/:districtId with Authorization token.'
      )
    }

    await District.update({...district, isActive: true}, {where: {id: district.id}})
  })
...
export const disableDistrictAddressingCertification = async districtID => {
  ...
  const cfg = await getDistrictConfigJson(districtID)
  if (cfg && Object.prototype.hasOwnProperty.call(cfg, 'certificate')) {
    await upsertDistrictConfigMerge(districtID, {certificate: null})
    return District.findByPk(districtID)
  }
...
export const enableDistrictAddressingCertification = async (districtID, certificateType) => {
  ...
  await upsertDistrictConfigMerge(districtID, {certificate: certificateType})
  return District.findByPk(districtID)
}
...
export const patchDistricts = async districts => {
  const bulkOperations = districts.map(async district => {
    if (Object.prototype.hasOwnProperty.call(district, 'config')) {
      throw new Error(
        'District "config" cannot be updated via PATCH /district. Use PATCH /api/district-config/:districtId with Authorization token.'
      )
    }
