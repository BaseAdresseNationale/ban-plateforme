...
app.patch('/addressing-certification/enable',
  ...
  async (req, res) => {
    try {
      ...
      console.log(':::::: force republication ::::::', `${API_IDFIX_URL}/compute-from-cog/${district?.meta?.insee?.cog}?force=true`);
      
      // local macos
      // console.log(':::::: force republication ::::::', `http://host.docker.internal:3000/compute-from-cog/${district?.meta?.insee?.cog}?force=true`);
      
      // Async Call to ID-Fix
      // local macos
      // fetch(`http://host.docker.internal:3000/compute-from-cog/${district?.meta?.insee?.cog}?force=true`, {
      fetch(`${API_IDFIX_URL}/compute-from-cog/${district?.meta?.insee?.cog}?force=true`, {
        method: 'GET',
        headers: {
