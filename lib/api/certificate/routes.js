...
import {
  ...
  countCertificatesByDistrict,
  countCertificatesByDistrictPerMonth
} from './models.js'
...
/**
 * @swagger
 * /api/certificate/stats/{districtCog}:
 *   get:
 *     summary: Nombre de certificats émis pour une commune
 *     description: Permet de récupérer le nombre de certificats téléchargés pour une commune
 *     tags:
 *       - 📜 Certificats d’adressage
 *     parameters:
 *       - in: path
 ...
 *     responses:
 *       200:
 *         description: Nombre de certificats émis pour la commune.
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *                   type: string
 *                   description: A message indicating the certificate was retrieved.
 *                   example: "Certificate count retrieved"
 *                 response:
 *                   $ref: '#/components/schemas/TYPE_json_ban_certificate_count'
 *       400:
 *         description: The request is invalid. The identifier is missing or incorrect.
 *         content:
 *           application/json:
 ...
 *                   type: string
 *                   description: A message certificate the certificate was not found.
 *                   example: "Request District Cog unknown"
 *                 response:
 *                   type: object
 *                   description: An empty object or additional error information.
 *       500:
...
app.get('/stats/:districtCog', async (req, res) => {
  try {
    const {districtCog} = req.params
    if (!districtCog) {
      handleAPIResponse(res, 400, 'districtCog is required', {})
      return
    }

    const count = await countCertificatesByDistrict(districtCog)
    handleAPIResponse(res, 200, 'Certificate count retrieved', {count})
  } catch (error) {
    console.error(error)
    handleAPIResponse(res, 500, 'Internal server error', {})
  }
...
/**
 * @swagger
 * /api/certificate/stats/{districtCog}/{year}:
 *   get:
 *     summary: Nombre de certificats émis pour une commune par mois
 *     description: Permet de récupérer le nombre de certificats téléchargés réparti par mois pour une commune
 *     tags:
 *       - 📜 Certificats d’adressage
 *     parameters:
 *       - in: path
 ...
 *     responses:
 *       200:
 *         description: Répartition mensuelle des certificats émis pour la commune.
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *                   example: "Certificate count retrieved"
 *                 response:
 *                   $ref: '#/components/schemas/TYPE_json_ban_certificate_count_by_month'
 *       400:
 *         description: The request is invalid. The identifiers are missing or incorrect.
 *         content:
 *           application/json:
 *             schema:
 *               type: object
...
app.get('/stats/:districtCog/:year', async (req, res) => {
  try {
    const {districtCog, year} = req.params
    if (!districtCog) {
      handleAPIResponse(res, 400, 'districtCog is required', {})
      return
    }
    if (!year) {
      handleAPIResponse(res, 400, 'year is required', {})
      return
    }

    const counts = await countCertificatesByDistrictPerMonth(districtCog, Number(year))
    handleAPIResponse(res, 200, 'Certificate count retrieved', counts)
  } catch (error) {
    console.error(error)
    handleAPIResponse(res, 500, 'Internal server error', {})
  }
