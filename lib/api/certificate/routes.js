...
/**
 * @swagger
 * /api/certificate/stats/{cog}:
 *   get:
 *     summary: Nombre de certificats émis pour une commune
 *     description: Permet de récupérer le nombre de certificats générés pour une commune
 *     tags:
 *       - 📜 Certificats d’adressage
 *     parameters:
 *       - in: path
...
app.get('/stats/:cog', async (req, res) => {
  try {
    const {cog} = req.params
    if (!cog) {
      handleAPIResponse(res, 400, 'cog is required', {})
      return
    }

    const count = await countCertificatesByDistrict(cog)
...
/**
 * @swagger
 * /api/certificate/stats/{cog}/{year}:
 *   get:
 *     summary: Nombre de certificats émis pour une commune par mois
 *     description: Permet de récupérer le nombre de certificats générés réparti par mois pour une commune
 *     tags:
 *       - 📜 Certificats d’adressage
 *     parameters:
 *       - in: path
...
app.get('/stats/:cog/:year', async (req, res) => {
  try {
    const {cog, year} = req.params
    if (!cog) {
      handleAPIResponse(res, 400, 'cog is required', {})
      return
    }
    ...
    const counts = await countCertificatesByDistrictPerMonth(cog, Number(year))
