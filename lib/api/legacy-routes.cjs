...
const API_IDFIX_URL
  = process.env.API_IDFIX_URL
  || 'https://plateforme.adresse.data.gouv.fr/api-idfix'
const API_IDFIX_TOKEN = process.env.API_IDFIX_TOKEN || ''
...
app.post(
  ...
  w(async (req, res) => {
    ...
    try {
      ...
      if (API_IDFIX_URL) {
        fetch(`${API_IDFIX_URL}/district/${codeCommune}/`, {
          method: 'GET',
          headers: {
...
app.get(
  '/ban/communes/:codeCommune/download/csv-bal/adresses',
  w(async (req, res) => {
    const {codeCommune} = req.params
    const {voies, numeros} = await getCommuneData(codeCommune)
...
app.post('/ban/stats', w(async (req, res) => {
  ...
  const codesCommuneArr = codesCommune || []
  try {
    if (codesCommuneArr.length > 0) {
      // If we have a list of communes we compute on the fly the stats for the selected communes
      const stats = await computeStats(codesCommuneArr)
      res.send(stats)
    } else {
      // If we don't have to filter on specific communes, return the pre-computed stats for all France
