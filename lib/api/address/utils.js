...
import {getAddressesByFilters, getAddressesForDeltaReport} from './models.js'
...
export const getDeltaReport = async (addressIDsWithHash, districtID) => {
  ...
  const existingAddresses = await getAddressesForDeltaReport({id: addressIDs}, ['districtID'])
  const existingAddressesMap = new Map(existingAddresses.map(({id, hash, isActive, districtID}) => [id, {hash, isActive, districtID}]))
  ...
  // Get only the ACTIVE addresses of the district from the db.
  // Inactive addresses are already soft-deleted and would never be re-deleted here,
  // so loading them (millions of historical rows on large districts) is useless and
  // is the main memory cost that made this endpoint crash the API pod.
  const activeAddressesFromDistrict = await getAddressesForDeltaReport({districtID, isActive: true})
  const addressIDsWithHashMap = new Map(addressIDsWithHash.map(({id, hash}) => [id, hash]))
  for (const {id} of activeAddressesFromDistrict) {
    // If an active address of the district is not in the request, we need to delete it
