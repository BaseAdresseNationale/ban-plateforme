...
export const getDeltaReport = async (addressIDsWithHash, districtID) => {
  ...
  const allAddressesFromDistrict = await getAddressesForDeltaReport({districtID})
  const allAddressesFromDistrictMap = new Map(allAddressesFromDistrict.map(({id, hash, isActive}) => [id, {hash, isActive}]))
