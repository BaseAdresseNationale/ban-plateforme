...
import {getCommonToponymsByFilters, getCommonToponymsForDeltaReport} from './models.js'
...
export const getDeltaReport = async (commonToponymIDsWithHash, districtID) => {
  ...
  const existingCommonToponyms = await getCommonToponymsForDeltaReport({id: commonToponymIDs}, ['districtID'])
  const existingCommonToponymsMap = new Map(existingCommonToponyms.map(({id, hash, isActive, districtID}) => [id, {hash, isActive, districtID}]))
  ...
  // Get only the ACTIVE common toponyms of the district from the db.
  const activeCommonToponymsFromDistrict = await getCommonToponymsForDeltaReport({districtID, isActive: true})
  const commonToponymIDsWithHashMap = new Map(commonToponymIDsWithHash.map(({id, hash}) => [id, hash]))
  for (const {id} of activeCommonToponymsFromDistrict) {
    // If an active common toponym of the district is not in the request, we need to delete it
