...
export const getDeltaReport = async (commonToponymIDsWithHash, districtID) => {
  ...
  for (const {id, hash} of commonToponymIDsWithHash) {
    if (existingCommonToponymsMap.has(id)) {
      ...
      if (currentCog.slice(0, 2) !== existingCog.slice(0, 2)) {
        console.log(`Warning:Warning: Common toponym with ID ${id} belongs to a different district (current: ${currentCog}, existing: ${existingCog}). This addition will be ignored as it already exists elsewhere.`)
      } else {
