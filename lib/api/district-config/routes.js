...
import {parseConfigPatch} from '../district/schema.js'
  ...
  .patch(auth, analyticsMiddleware, async (req, res) => {
    try {
      if (!isCertificateGenerationEnabled()) {
        handleAPIResponse(res, 403, CERTIFICATE_MAINTENANCE_MESSAGES.districtConfig, {})
        return
      }
      ...
      let validated
      try {
        validated = await parseConfigPatch(req.body)
      } catch (error) {
        if (error?.name === 'ValidationError') {
          handleAPIResponse(res, 400, 'Invalid config format', error.errors)
          return
        }
      ...
      await upsertDistrictConfigMerge(districtId, validated)
      const config = await getDistrictConfigJson(districtId)
