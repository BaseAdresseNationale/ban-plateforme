...
import {getLatestAlerts, getLatestWarnings, getAlertStatusForRevisionsBatch, regexIsCodeInsee} from './utils.js'
...
async function fetchCurrentRevisionsCached() {
  ...
  if (currentRevisionsFetchPromise) {
  ...
  currentRevisionsFetchPromise = (async () => {
    try {
      ...
      const byCog = new Map()
      for (const r of raw) {
        const cog = (r.codeCommune === null || r.codeCommune === undefined) ? null : String(r.codeCommune).padStart(5, '0')
        if (cog === null || cog === '' || !regexIsCodeInsee.test(cog)) {
          continue
        }
...
export function computeStatutFromRevisions(idRevisionStocke, revisions, alertStatus = null) {
  ...
  const idStocke = (idRevisionStocke === null || idRevisionStocke === undefined) ? null : String(idRevisionStocke)
  const currentId = (currentRevision.id === null || currentRevision.id === undefined) ? null : String(currentRevision.id)
...
export async function getStatutsForCogs(cogs) {
  const list = cogs.filter(c => typeof c === 'string' && regexIsCodeInsee.test(c))
  ...
  for (const r of communesRows || []) {
    ...
    if (!regexIsCodeInsee.test(cog)) {
      continue
    }

    idRevisionByCog.set(cog, (r.idRevision === null || r.idRevision === undefined) ? null : String(r.idRevision))
    ...
    communeByCog.set(cog, {
      ...
      idRevision: (r.idRevision === null || r.idRevision === undefined) ? null : String(r.idRevision),
