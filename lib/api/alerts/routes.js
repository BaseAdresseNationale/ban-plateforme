...
import {getCommuneStatus, createSubscriber, sendWebhookNotifications, getLatestAlerts, getLatestWarnings, regexIsCodeInsee} from './utils.js'
...
app.post('/subscribers', auth, analyticsMiddleware, async (req, res) => {
  try {
    if (!isCertificateGenerationEnabled()) {
      handleAPIResponse(res, 403, CERTIFICATE_MAINTENANCE_MESSAGES.adminOptions, {})
      return
    }
...
app.post('/statuts-communes', analyticsMiddleware, async (req, res) => {
  try {
    ...
    if (Array.isArray(body.cogs)) {
      cogs = body.cogs.map(String).filter(c => regexIsCodeInsee.test(c))
    } else if (Array.isArray(body.communes)) {
      cogs = body.communes
        .map(c => (c && typeof c.cog === 'string' ? c.cog : null))
        .filter(c => c && regexIsCodeInsee.test(c))
    }
...
app.get('/statuts-communes', analyticsMiddleware, async (req, res) => {
  try {
    ...
    const cogs = cogsParam.split(',').map(s => s.trim()).filter(c => regexIsCodeInsee.test(c))
    if (cogs.length === 0) {
