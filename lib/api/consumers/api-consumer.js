...
import {decrementPipelinePending, hasPipelinePending} from '../pipeline-district-lock.js'
...
const scheduleExportForDistricts = async (dataType, jobType, data, jobStatus) => {
  ...
  await Promise.all(uniqueRelatedDistrictIDs.map(async districtID => {
    if (await hasPipelinePending(districtID)) {
      const remaining = await decrementPipelinePending(districtID)
      if (remaining === 0) {
    ...
    if (jobStatus && jobStatus.status === 'success') {
      await addOrUpdateJob(
        exportToExploitationDBQueue,
        districtID,
        exportToExploitationDBJobDelay
...
export default async function apiConsumers({data: {dataType, jobType, data, statusID}}, done) {
  try {
    ...
    if (!dataType || !jobType || !data || !statusID) {
      console.warn('Job ignoré - variables manquantes:', {dataType, jobType, data: Boolean(data), statusID})
      done()
      return
    }

    if (!Array.isArray(data)) {
      console.warn('Job ignoré - data n\'est pas un array:', typeof data)
      done()
      return
    }
    ...
    await scheduleExportForDistricts(dataType, jobType, data, jobStatus)
  } catch (error) {
    console.error('Erreur dans apiConsumers:', error)
    ...
    if (statusID) {
      try {
        ...
        console.error('Impossible de logger l\'erreur:', statusError)
      }
    }

    // Erreur permanente : décrémenter pending pipeline (job terminé côté Bull, sans retry)
    if (!TRANSIENT_DB_ERRORS.has(error.name) && dataType && jobType && data) {
      await scheduleExportForDistricts(dataType, jobType, data, {status: 'error'})
    }

    // Erreur transiente Postgres → done(error) déclenche le retry Bull (×3 avec backoff)
    // Erreur permanente (contrainte unique, données invalides) → done() sans retry
    if (TRANSIENT_DB_ERRORS.has(error.name)) {
      done(error)
      return
    }
  }
