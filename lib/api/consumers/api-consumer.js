...
export default async function apiConsumers(job, done) {
  try {
    let dataType, jobType, data, statusID

    // CAS 1: Structure actuelle - job.data contient directement les données
    if (job.data && job.data.dataType) {
      ({dataType, jobType, data, statusID} = job.data)
    }
    // CAS 2: Structure ancienne - job.data.data contient les vraies données  
    else if (job.data && job.data.data && job.data.data.dataType) {
      ({dataType, jobType, data, statusID} = job.data.data)
    }
    // CAS 3: job.data est un string JSON
    else if (typeof job.data === 'string') {
      const parsedData = JSON.parse(job.data)
      ({dataType, jobType, data, statusID} = parsedData)
    }
    // CAS d'erreur
    else {
    ...
    if (!dataType || !jobType || !statusID) {
      throw new Error('Variables manquantes dans les données reçues')
    }
