...
export default async function apiConsumers({data: {dataType, jobType, data, statusID}}, done) {
  try {
    console.log('DEBUG - Variables reçues:')
    ...
    // Si data est undefined, c'est qu'il faut parser différemment
    if (!data || !dataType || !jobType || !statusID) {
      console.log('ERREUR: Variables undefined, vérifiez la structure des données')
