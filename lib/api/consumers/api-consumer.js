...
export default async function apiConsumers(job, done) {
  try {
    console.log('=== DEBUG COMPLET ===')
    console.log('1. job complet:', job)
    console.log('2. job.data:', job.data)
    console.log('3. typeof job.data:', typeof job.data)
    
    // Essayer différentes structures possibles
    if (job.data && job.data.dataType) {
      // Structure: job.data = {dataType, jobType, data, statusID}
      console.log('4. CAS A: Structure normale')
      finalDataType = job.data.dataType
      finalJobType = job.data.jobType
      finalData = job.data.data
      finalStatusID = job.data.statusID
    } else if (typeof job.data === 'string') {
      // Structure: job.data = JSON string
      console.log('4. CAS B: JSON string')
      const parsed = JSON.parse(job.data)
      finalDataType = parsed.dataType
      finalJobType = parsed.jobType
      finalData = parsed.data
      finalStatusID = parsed.statusID
    } else {
      console.log('4. CAS C: Structure inconnue')
      console.log('job.data keys:', Object.keys(job.data || {}))
      throw new Error('Structure de données non reconnue')
    }
    
    console.log('5. Variables finales:')
    console.log('finalDataType:', finalDataType)
    console.log('finalJobType:', finalJobType) 
    console.log('finalStatusID:', finalStatusID)
    console.log('finalData:', finalData)
    console.log('=== FIN DEBUG ===')
    
    if (!finalData || !finalDataType || !finalJobType || !finalStatusID) {
      throw new Error('Variables manquantes après parsing')
    }
