...
function adresseToFeature(a) {
  return {
    type: 'Feature',
    properties: pick(a, 'numero', 'suffixe', 'codeVoie', 'nomVoie', 'codeCommune', 'nomCommune', 'sources', 'positionSource', 'positionType', 'id', 'codeAncienneCommune', 'nomAncienneCommune'),
    geometry: a.position
  }
}
...
async function writeData(path, adresses) {
  ...
  const steps = [
    intoStream.object(adresses.map(adresseToFeature)),
