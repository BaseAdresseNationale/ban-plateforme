...
const {computeBufferedBbox, getCentroidFromPoints, derivePositionProps} = require('../util/geo')
...
async function consolidateVoies(adressesCommuneWithGroupId) {
  ...
  const voies = chain(adressesCommuneWithGroupId)
    ...
    .map(adresses => {
      ...
      const centroid = getCentroidFromPoints(positions)
      const {lon, lat, x, y, tiles} = derivePositionProps(centroid)
