...
async function buildVoies(sourcesData, {codeCommune, pseudoCodeVoieGenerator, forceCertification}) {
  const balData = sourcesData.find(d => d.source === 'bal')

  if (balData && balData.adresses.length > 0) {
    return buildVoiesBAL(
      balData.adresses,
      {codeCommune, pseudoCodeVoieGenerator, forceCertification}
    )
  }
...
async function composeCommune(codeCommune) {
  ...
  const composedVoies = await buildVoies(sourcesData, {
    codeCommune,
