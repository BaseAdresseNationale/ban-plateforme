...
async function composeCommune(codeCommune) {
  ...
  const communeRecord = {
    ...
    numerosCount: voies.reduce((acc, voie) => acc + voie.numerosCount, 0),
    voiesCount: voies.length,
    lieuxDitsCount: lieuxDits.length,
    compositionType: voies.some(v => v.sourceNomVoie === 'bal') ? 'bal' : 'assemblage'
  }
