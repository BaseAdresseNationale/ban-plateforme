...
async function composeCommune(codeCommune) {
  ...
  const composedVoies = isBAL
    ? await BAL.buildVoies(balData.adresses, composeVoiesOptions)
    : await MS.buildVoies(flatten(sourcesData.map(d => d.adresses)), composeVoiesOptions)
  ...
  const lieuxDits = isBAL
    ...
    : await MS.buildLieuxDits(sourcesData, buildLieuxDitsOptions)
