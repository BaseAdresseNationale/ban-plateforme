...
async function composeCommune(codeCommune, ignoreIdConfig) {
  try {
    ...
    if (!compositionOptions.force && currentRevision && commune?.idRevision === currentRevision?.id) {
    ...
    const districtID = await getDistrictIDFromMainDB(codeCommune)
    if (!districtID) {
