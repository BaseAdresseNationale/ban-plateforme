...
async function importFromApiDepot(revision) {
  ...
  return validationResult.rows.filter(r => !isErroredRow(r)).map(row => {
    const adresse = {
      ...
      nomVoie: row.parsedValues.toponyme || row.parsedValues.voie_nom,
      toponyme: row.parsedValues.toponyme,
      lieuDitComplementNom: row.parsedValues.lieudit_complement_nom,
      parcelles: row.parsedValues.cad_parcelles || [],
      codeCommune: row.parsedValues.commune_insee || row.additionalValues?.cle_interop?.codeCommune, // commune_insee est prioritaire sur codeCommune et obligatoire 1.5 pas de risque de crache si cle_interop est vide
      ...
      nomVoieAlt: row.localizedValues?.toponyme || row.localizedValues?.voie_nom || {},
