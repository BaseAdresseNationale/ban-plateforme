...
function buildCertifie({certificationCommune, sourcePosition}) {
  if (sourcePosition === 'ign-api-gestion-municipal_administration') {
    return true
  }

  if (sourcePosition === 'bal') {
    return Boolean(certificationCommune)
  }
...
function consolidateNumero(numeroAdresses, {idVoieFantoir, codeCommune}, {selectPosition, isBAL}) {
  ...
  const certifie = buildCertifie({certificationCommune, sourcePosition})
