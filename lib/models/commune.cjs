...
async function getPostalCode(banId) {
  const doc = await mongo.db.collection('numeros').findOne(
    {banId},
    {projection: fieldsToProj(['codePostal'])}
  )
  return doc?.codePostal
}
...
async function getCommunesSummaryByCogs(cogs) {
  if (!Array.isArray(cogs) || cogs.length === 0) {
    return []
  }

  const fields = ['codeCommune', 'nomCommune', 'dateRevision', 'idRevision', 'nbVoies', 'nbNumeros', 'nbLieuxDits', 'nbNumerosCertifies']
  const list = await mongo.db.collection('communes')
    .find({codeCommune: {$in: [...new Set(cogs)]}}, {projection: fieldsToProj(fields)})
    .toArray()
  return list
}
...
async function getPopulatedCommune(codeCommune) {
  ...
  if (!commune.banId) {
    ...
    const districts = await getDistrictsFromCog(codeCommune)
    const district = districts?.[0]
    if (district?.id) {
      commune.banId = district.id
    }
  }
...
async function getAdressesFeatures(z, x, y) {
  ...
  const features = numeros.flatMap(n => {
