...
async function enableCommuneAddressingCertification(codeCommune) {
  if (!codeCommune) {
    throw new Error('Code commune is required')
  }

  const communesCollection = mongo.db.collection('communes')
  const commune = await communesCollection.findOne({codeCommune})

  if (!commune) {
    throw new Error(`Commune ${codeCommune} not found`)
  }

  const update = commune.config === null
    ? {config: {certificate: {}}}
    : {'config.certificate': {}}

  const result = await communesCollection.updateOne(
    {codeCommune},
    {$set: update}
  )
