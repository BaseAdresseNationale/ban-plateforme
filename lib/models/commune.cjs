...
async function saveCommuneData(codeCommune, {commune, voies, numeros}) {
  ...
  const computPseudoIdVoie = (pseudoCodeVoieGenerator, voie) => {
    if(!pseudoCodeVoieGenerator)  throw new Error('pseudoCodeVoieGenerator is not defined')
  ...
  if(allCodesCommunes && allCodesCommunes.length > 0) {
    ...
    numeros.forEach(numero => {
      const { codeCommune, idVoie } = numero
      ...
      if(allIdsNumeros.has(numero.id)) {
        const newIdNumero = computeIdNumero(codeCommune, newIdVoie, numero)
        numero.idVoie = newIdVoie
        numero.id = newIdNumero
        // numero.cleInterop = newIdNumero
