...
async function enableCommuneAddressingCertification(codeCommune) {
  if (!codeCommune) {
    throw new Error('Code commune is required to enable addressing certification')
  }

  const commune = await mongo.db.collection('communes').findOne({codeCommune})
  if (!commune) {
    throw new Error(`Commune with code ${codeCommune} not found`)
  }

  const result = await mongo.db.collection('communes').updateOne(
    {codeCommune},
    {$set: {'config.certificate': {}}}
  )
...
module.exports = {
  askComposition,
  enableCommuneAddressingCertification,
  getAskedComposition,
  askCompositionAll,
  finishComposition,
  getCommune,
