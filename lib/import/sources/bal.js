...
const codesLR = require('@ban-team/shared-data/langues-regionales.json')
...
function prepareData(item) {
  ...
  const nomAlt = []
  const LDCNAlt = []
  let nomAltDefined = false
  let LDCNDefined = false
  ...
  for (const {code} of codesLR) {
    ...
    if (item[nomChamp]) {
      nomAltDefined = true
      nomAlt[code] = item[nomChamp]
    }

    if (item[nomLDCN]) {
      LDCNDefined = true
      LDCNAlt[code] = item[nomLDCN]
    }
  }
