...
function prepareData(item) {
  const adresse = {
    ...
    // Faire comme nomVoie pour lieuDitComplementNom_CLR
  ...
  // item.nomChamp est undefined... 
  for (const code of codesLR) {
    const nomChamp = 'nomVoie_' + code
    console.log('nomchamp', nomChamp)
    console.log('nomitem', item.nomChamp)
    if (item.nomChamp) {
      adresse.nomChamp = item.nomChamp
    }
  }
