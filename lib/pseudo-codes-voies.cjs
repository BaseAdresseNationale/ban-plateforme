...
const PseudoCodeVoie = require('./models/pseudo-code-voie.cjs')
const {generateBase36String} = require('./util/base36.cjs')
...
function selectBestResult(results, ctx) {
  return minBy(results, r => leven(ctx.normalizedString, r.stringContext.normalizedString))
}
...
async function createPseudoCodeVoieGenerator(codeCommune) {
  ...
  function findItem(nomVoie, codeAncienneCommune, forceScope = false) {
    ...
    // SIGNIFICANT WORDS
    const significantWordsCompareResults = scopedItems.filter(item => item.stringContext.significantWordsString === ctx.significantWordsString)
    ...
    // PHONETIC
    const phoneticCompareResults = scopedItems.filter(item => item.stringContext.phoneticString === ctx.phoneticString)
    ...
    // OVERLAPPING
    const overlapCompareResults = scopedItems.filter(item => overlap(item.stringContext.significantWords, ctx.significantWords))
