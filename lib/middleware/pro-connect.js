...
export const checkSessionFromRequest = async body => {
  try {
    ...
    const {error: schemaError, value: validatedData} = banSessionSchema.validate(data)
    if (schemaError) {
      return {
        error: 400,
        message: 'Format de données de session invalide : ' + schemaError.message
      }
    }

    const session = await getSession(validatedData)
    if (!session) {
      return {
        error: 401,
